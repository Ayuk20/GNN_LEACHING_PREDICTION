...
# There was a shape mismatch error here reason I commented this.
...
# ============================================================
# Full hetero-GAT query graph builder
# ============================================================
...
# ============================================================
# Global KG embedding recommender
# ============================================================
class GlobalKGEmbeddingRecommender:
    def __init__(self, ref_df, exp_embeddings):
        self.ref_df = ref_df.copy().reset_index(drop=True)
        self.exp_embeddings = exp_embeddings

        if len(self.ref_df) != len(self.exp_embeddings):
            raise ValueError("Reference dataframe length and embedding count do not match.")
    ...
    def approximate_query_embedding(self, row, k_seed=15):
        # approximate query embedding from nearest structured experiments
        vals = np.array([[
        ...
        q = self.scaler.transform(vals)
        sims = cosine_similarity(q, self.X_num)[0]

        df = self.ref_df.copy()
        df["seed_similarity"] = sims

        # soft categorical preference
        df["seed_similarity_adj"] = df["seed_similarity"]
        df.loc[df["material"] == row["material"], "seed_similarity_adj"] += 0.05
        df.loc[df["ree_class"] == row["ree_class"], "seed_similarity_adj"] += 0.03
        df.loc[df["solution"] == row["solution"], "seed_similarity_adj"] += 0.02

        top_idx = df.sort_values("seed_similarity_adj", ascending=False).head(k_seed).index.to_numpy()
        weights = df.loc[top_idx, "seed_similarity_adj"].to_numpy()
        weights = np.clip(weights, 1e-8, None)
        weights = weights / weights.sum()

        query_emb = np.sum(self.exp_embeddings[top_idx] * weights[:, None], axis=0)
        return query_emb

    def retrieve_similar(self, row, k=8):
        q_emb = self.approximate_query_embedding(row)
        sims = cosine_similarity(q_emb.reshape(1, -1), self.exp_embeddings)[0]

        df = self.ref_df.copy()
        df["kg_similarity"] = sims

        # slight preference for same material / ree
        df["kg_similarity_adj"] = df["kg_similarity"]
        df.loc[df["material"] == row["material"], "kg_similarity_adj"] += 0.05
        df.loc[df["ree_class"] == row["ree_class"], "kg_similarity_adj"] += 0.03

        return df.sort_values("kg_similarity_adj", ascending=False).head(k).copy()
    ...
    def recommend_changes(self, row, better_df, top_n=3):
        ...
        for var in actionable_num:
            ...
            target = float(np.median(better_df[var].values))
            rel_gap = abs(target - current) / (abs(current) + 1e-6)
...
# ============================================================
class FullHeteroGATPlusGlobalKGAttentionFramework:
    def __init__(
        self,
        ...
        # checks
        for p in [data_csv, full_model_path, global_kg_graph_path, global_kg_model_path]:
        ...
        # full hetero-GAT predictor
        self.full_model = REEHeteroGAT(
        ...
        self.full_model.load_state_dict(full_state)
        self.full_model.eval()

        # global KG attention model
        self.global_data = torch.load(global_kg_graph_path, map_location="cpu", weights_only=False)
        self.global_data = self.global_data.to(self.device)

        self.global_model = GlobalKGHeteroGAT(
            metadata=self.global_data.metadata(),
            hidden_dim=hidden_dim,
            heads=heads,
            dropout=dropout,
        ).to(self.device)

        global_state = torch.load(global_kg_model_path, map_location="cpu", weights_only=False)
        self.global_model.load_state_dict(global_state)
        self.global_model.eval()

        with torch.no_grad():
            self.global_embeddings = self.global_model.get_experiment_embeddings(self.global_data)

        self.recommender = GlobalKGEmbeddingRecommender(self.df, self.global_embeddings)
    ...
    def predict_recovery(self, user_input):
        ...
        with torch.no_grad():
            pred = self.full_model(graph).item()
        ...
        result = {
            ...
            "similar_neighbors": neighbors[
                [
                    ...
                    "si", "al", "fe", "temperature", "ph",
                    "solid_liquid_ratio", "stirring_speed", "leaching_time",
                    "recovery", "kg_similarity", "kg_similarity_adj"
                ]
...
if __name__ == "__main__":
    ...
    framework = FullHeteroGATPlusGlobalKGAttentionFramework(
        data_csv="ree_leaching_gnn_ready.csv",
        full_model_path="ree_heterogat_full_model.pt",
        global_kg_graph_path="ree_global_kg.pt",
        global_kg_model_path="global_kg_heterogat_model.pt",
    ...
    print(sim_df[["material", "solution", "ree_class", "recovery", "kg_similarity_adj"]].head(5))